...
{
  BOOST_REQUIRE_EQUAL(std::string("TestPluginBase"), PluginTypeDeducer<cettest::TestPluginBase>::value);
  BOOST_REQUIRE_EQUAL(pf.pluginType("TestPlugin"), PluginTypeDeducer<cettest::TestPluginBase>::value);
}
...
BOOST_AUTO_TEST_CASE(CheckFinder)
{
  ...
  BOOST_REQUIRE(fptr);
  BOOST_REQUIRE_EQUAL(fptr(), PluginTypeDeducer<cettest::TestPluginBase>::value);
